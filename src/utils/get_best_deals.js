...
const getRecommendedPrice = require('../../src/utils/get_recommended_price');
				...
				const afterGetPrice = async (price) => {
					...
					if (item.isOtherVendor == 'Y') {
						...
						await oldItem.images.forEach((imgItem) => {
							...
							let oldThumbImg = imgItem.thumbImage;
							if (oldImg) {
							...
							if (oldThumbImg) {
								let newThumbImg = oldThumbImg.replace(
									'https://demo-bucket-c2c-001.s3.amazonaws.com/',
									'https://d1tl44nezj10jx.cloudfront.net/'
								);
								imgItem.thumbImage = newThumbImg;
							}
						});

						if (oldItem.images.length > 0) {
							oldItem.defaultImage['fullImage'] =
								oldItem.images[0].thumbImage || oldItem.images[0].fullImage;
						} else {
		...
		const afterGetingBestDeals = async (bestDeals) => {
			...
			finalBestDeals.forEach((item, index) => {
				if (!item.images.length) {
					...
					finalBestDeals[index].imagePath =
						item.images[0].thumbImage || item.images[0].fullImage;
				}
